// IMPORTANT: we deliberately use `as const` object literals instead of TypeScript
// `enum` keyword. Reason: esbuild (used by Vite in dev mode) transpiles `enum`
// declarations into an IIFE-style construct that does not always survive native
// ESM module re-exports across workspace package boundaries — Vite ends up
// serving an `enums.js` module without the expected named exports, breaking
// `import { Role } from '@telemed/shared-types'` in the browser.
//
// `as const` objects are plain ESM named exports and work everywhere.
// We pair each object with a `type Foo = typeof Foo[keyof typeof Foo]` line so
// the value and the type share the same name (the same ergonomics as `enum`).
